...
import GriptokTab from "../components/GriptokTab";

function Detailgt(props) {
  let { id } = useParams();
  let product = props.griptok.find((x)=>x.id == id);
  ...
  useEffect(() => {
    ...
    let item  = watchedItem.indexOf('/img/gtdt'+(product.id+1)+'.png');
    if(item !== -1){ watchedItem.splice(item,1); }
    watchedItem.push('/img/gtdt'+(product.id+1)+'.png')
